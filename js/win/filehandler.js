...
class FileHandler
{
    ...
    saveFile (file)
    {
        ...
        _k_.dbg(".", 225, 8, null, 'File.save',file)
        return File.save(file,editor.text(),function (err, saved)
        {
            _k_.dbg(".", 229, 12, null, 'File.saved',err,file)
            editor.saveScrollCursorsAndSelections()
            if (!_k_.empty(err))
            {
